<x-app-layout>
    <x-slot name="header">{{ __('Quiz Dünzenleme Sayfasındasınız') }}</x-slot>
    <div class="sm:pt-14">
        <div class="card col-md-6 mx-auto overflow-hidden bg-white shadow sm:rounded-lg">
            <div class="card-body">
                ...
                @if (request()->routeIs('*add-question*'))
                    <form method="POST" action="{{route('admin.add-question.update',[$quiz,$quiz->id])}}" class="needs-validation" novalidate>
                        @elseif(request()->routeIs('*quizzes*'))
                            <form method="POST" action="{{route('admin.quizzes.update',$quiz)}}" class="needs-validation" novalidate>
                                @endif
                                @csrf @method('put')
                                {{--Title--}}
                                ...
                                <div class="form-floating mt-2" id="divDesc">
                                ...
                                <div class="row g-2">
                                    <div class="col-md">
                                        ...
                                        <div class="form-floating mt-2">
                                            ...
                                            @if($quiz->quiz_list_count <= 4)
                                                <option disabled class="text-danger" value="">Aktif edilemez. Soru Sayısı yetersiz...</option>
                                            @else
                                                <option @if(old('status') === 'active' or $quiz->status === 'active' && $quiz->quiz_list_count >= 4) selected @endif value="active">Aktif</option>
                                            @endif
                                    ...
                                    <div class="col-md">
                                        ...
                                        <div class="form-floating mt-2" id="divFinis">
                                ...
                                <div class="d-flex justify-content-end mt-2">
                                    <button type="submit" name="submit" value="{{$submit}}" class="btn btn-success">{{ __('Düzenle') }}</button>
                                </div>
                            </form>
            </div>
        </div>
